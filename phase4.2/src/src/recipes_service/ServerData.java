...
public class ServerData {
	...
	public synchronized void removeRecipe(String recipeTitle) {
		// esta función es muy parecida a addRecipe
		Timestamp timestamp = nextTimestamp();
	...
	//=========
	// Recibe un MessageOperation y, según el tipo de operación que contenta, añade o elimina una receta
	public synchronized void recipeManager(MessageOperation mso) {
		...
		if (tipoOp == OperationType.ADD) {
			//System.out.println(">>> Ejecutada función ServerData.recipeManager(add)");
			AddOperation op = ((AddOperation) mso.getOperation());
			if (this.log.add(op)) this.recipes.add(op.getRecipe());		
		// Operación ELIMINAR
        } else {
        	//System.out.println(">>> Ejecutada función ServerData.recipeManager(remove)");
